...
JustRun.Game = {
    create: function() {
        ...
        this.combo = 0;
        this.timer2 = game.time.events.loop(
            1000,
            () => {
        ...
        this.timer3 = game.time.events.loop(
            405,
            () => (this.youCanShot = true),
            this
        );
        this.timer4 = game.time.events.loop(
            4000,
            () => this.messages.children.map(t => t.destroy()),
            this
        );
        this.timer5 = game.time.events.loop(1000, () => (this.combo = 0), this);
        this.timer6 = game.time.events.loop(
            10000,
    ...
    update: function() {
        this.shooters.fireballs.children
            .filter(fireball => fireball.x < 0)
            .map(fireball => fireball.destroy());
    ...
    die: function(fireball, player) {
        if (player.x > 50) {
            ...
            this.messages.add(
                game.add.text(player.x, player.y, this.combo + " KILL!", style)
            );
